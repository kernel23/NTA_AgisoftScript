...
for dir_with_jpg in dirs_with_jpg:
    ...
    chunk.buildDepthMaps(downscale = 2, filter_mode = Metashape.MildFiltering) #Build depth maps
    doc.save()
    
    has_transform = chunk.transform.scale and chunk.transform.rotation and chunk.transform.translation

    if has_transform:
        chunk.buildPointCloud() #Builds Point Cloud
        doc.save()
        
        
        chunk.buildDem(source_data=Metashape.PointCloudData) #Build the Digital Elevation Measurement 
        doc.save()
    ...
    if chunk.orthomosaic:
        orthomosaic_path = os.path.join(output_folder, dir_name + '_orthomosaic.tif')
        chunk.exportRaster(orthomosaic_path, source_data=Metashape.OrthomosaicData, resolution =0.08 )
